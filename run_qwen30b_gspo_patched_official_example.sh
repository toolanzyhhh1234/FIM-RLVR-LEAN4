# Patched official example to use HF repo name for model path.
set -x

mkdir -p verl_logs
log_ts=$(date +%Y%m%d-%H%M%S)
exec > >(tee -a "verl_logs/run_qwen30b_gspo_${log_ts}.log") 2>&1

export ARNOLD_WORKER_GPU=${ARNOLD_WORKER_GPU:-1}
