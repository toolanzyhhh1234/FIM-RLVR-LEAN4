...
def main():
    ...
    # Test with a simple known-good proof
    test_proof = """import Mathlib

theorem simple_test : 1 + 1 = 2 := by
  norm_num"""
    
    print("Testing simple proof...")
    success, output = verifier.verify(test_proof)
    print(f"Simple proof success: {success}")
    if not success:
        print(f"Error: {output[:200]}")
        return
    
    # Test with a NuminaMath-style proof (from your earlier example)
    numina_proof = """import Mathlib
...
theorem algebra_4013 {a b c : ℝ} (h : a * b * c = 1) (haux : 1 + a + a * b ≠ 0) :
  ...
  exact mul_inv_cancel₀ haux"""
    ...
    print("Testing NuminaMath-style proof...")
    success, output = verifier.verify(numina_proof)
    print(f"NuminaMath proof success: {success}")
    if not success:
        print(f"Error: {output[:500]}")
    else:
        print("✅ Verification environment works - can proceed with data processing")
