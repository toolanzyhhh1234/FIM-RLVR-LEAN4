...
def main() -> None:
    ...
    output_path = os.environ.get(
        "OPENROUTER_OUTPUT_PATH", "data/openrouter_synthetic.parquet"
    )
    mask_ratio = float(os.environ.get("FIM_MASK_RATIO", "0.15"))
    exclude_sorry = _bool_env("FIM_EXCLUDE_SORRY", True)
    dry_run = _bool_env("OPENROUTER_DRY_RUN", False)
    dry_run_path = os.environ.get(
        "OPENROUTER_DRY_RUN_PATH", "data/openrouter_synthetic_dryrun.jsonl"
    )
    target_default = "5" if dry_run else "1000"
    target_samples = int(os.environ.get("OPENROUTER_TARGET_SAMPLES", target_default))
    ...
    progress = tqdm(total=target_samples, desc="OpenRouter generated", unit="sample")
