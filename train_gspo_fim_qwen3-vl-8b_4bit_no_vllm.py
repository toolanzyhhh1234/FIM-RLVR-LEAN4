...
OUTPUT_DIR = "outputs_fim_grpo_qwen3_vl_32b"
...
LOAD_IN_4BIT = _bool_env("FIM_LOAD_IN_4BIT", True)  # Default to 4-bit QLoRA
FAST_INFERENCE = _bool_env("FIM_FAST_INFERENCE", False)  # Disable vLLM for 4-bit path
MAX_COMPLETION_LENGTH = _int_env("FIM_MAX_COMPLETION_LENGTH", 16384)
...
def _ensure_transformers_compat(model_name: str) -> None:
    ...
    if "qwen3-vl" in lowered:
        if Version(transformers_version) < Version("4.57.0"):
            raise RuntimeError(
                "This model requires transformers>=4.57.0. "
                "Update with `pip install --upgrade transformers`."
            )
...
def main():
    ...
    load_kwargs = {
        ...
        "gpu_memory_utilization": float(os.environ.get("FIM_GPU_MEMORY_UTIL", "0.8")),
    }
    if FAST_INFERENCE:
    ...
    # Add LoRA adapters (vision LLM setup per Qwen3-VL GRPO example)
